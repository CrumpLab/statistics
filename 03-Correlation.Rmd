...
Notice we have defined b in terms of m, so we first solve for m, then we can put it into the formula for b. In these formulas, the $x$ and the $y$ refer to the individual scores. Here's a table showing you how everything fits together. 
...
$intercept = b = \frac{\sum{y}-m\sum{x}}{n} = \frac{39 - 1.19*34}{7} = -.22 $


Great, now we can check our work, let's plot the scores in a scatterplot and draw a line through it with slope = -1.23, and a y-intercept of 1.36. It should go through the middle of the dots. Yup, they do!
